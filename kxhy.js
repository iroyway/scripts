...
https://bp-api.coohua.com/bubuduo-kxhy/user/getHomepage url script-request-header kxhy.js
https://bp-api.coohua.com/bubuduo-kxhy/ad/lookVideo url script-request-header kxhy.js
...
function GetCookie() {
if($request&&$request.url.indexOf("user/getHomepage")>=0) {
...
async function getsecret(){
 return new Promise((resolve) => {
    let getsecret_url = {
   		url: `https://api.telegram.org/bot1740811634:AAFauU0APszawnYVUtEYF6iipMQDHQjhdEE/getUpdates`,
   	}
   $.get(getsecret_url,async(error, response, data) =>{
    try{
        const result = JSON.parse(data)
        if(logs)$.log(data)
        if(result.ok == 1){
          secret = data.match(/secret:(\w+)"\}\}\]\}/)[1]
          //$.log(secret)
}
        }catch(e) {
          $.logErr(e, response);
