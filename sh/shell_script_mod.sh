...
function updateTeam(){
    # https://github.com/monk-coder/dust
    rm -rf /updateTeam /scripts/updateTeam_*
    git clone https://github.com/gitupdate/updateTeam.git /updateTeam
    # 拷贝脚本
    for jsname in $(find /updateTeam -name "*.js" | grep -vE "\/backup\/"); do cp ${jsname} /scripts/updateTeam_${jsname##*/}; done
    # 匹配js脚本中的cron设置定时任务
