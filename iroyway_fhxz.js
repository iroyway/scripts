...
const iroyway_fhxzurlArr = [], iroyway_fhxzhdArr = [], iroyway_fhxzcount = ''
let iroyway_fhxzurl = $.getdata('iroyway_fhxzurl')
let iroyway_fhxzhd = $.getdata('iroyway_fhxzhd')
let id = ""
//let tx=[{"type":"market_getItemList","data":{}}]
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        iroyway_fhxzhdArr.push($.getdata('iroyway_fhxzhd'))
        let iroyway_fhxzcount = ($.getval('iroyway_fhxzcount') || '1');
        for (let i = 2; i <= iroyway_fhxzcount; i++) {
            iroyway_fhxzurlArr.push($.getdata(`iroyway_fhxzurl${i}`))
            iroyway_fhxzhdArr.push($.getdata(`iroyway_fhxzhd${i}`))
        }
        console.log(`------------- 共${iroyway_fhxzhdArr.length}个账号-------------\n`)
        for (let i = 0; i < iroyway_fhxzhdArr.length; i++) {
            if (iroyway_fhxzhdArr[i]) {

                iroyway_fhxzurl = iroyway_fhxzurlArr[i];
                iroyway_fhxzhd = iroyway_fhxzhdArr[i];
                $.index = i + 1;
                console.log(`\n开始【富豪小镇提现${$.index}】`)
                await iroyway_fhxzxx();
                await $.wait(3000);
                await zdcj();
                await $.wait(3000);
        ...
        $.post(url, async (err, resp, data) => {
            ...
            try {
                ...
                if (result.code == 0) {
                    ...
                    console.log('\n富豪小镇用户信息获取成功\n当前梅子:' + result.data.marketItemList.stateCode + '\n当前可提现产品:' + result.data.marketItemList.title)
