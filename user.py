...
@client.on(events.NewMessage(chats=[-1001284907085, my_chat_id], pattern=r'.*店|.*豆.*|.*dou.*')) #张牧之频道
@client.on(events.NewMessage(chats=[-1001419355450, my_chat_id], pattern=r'.*店|.*豆.*|.*dou.*')) #动物园
@client.on(events.NewMessage(chats=[-1001285084157, my_chat_id], pattern=r'.*店|.*豆.*|.*dou.*')) #自己测试频道
@client.on(events.NewMessage(chats=[-1001197524983, my_chat_id], pattern=r'.*店|.*豆.*|.*dou.*'))
...
@client.on(events.NewMessage(chats=[-1001284907085, my_chat_id], pattern=r".*=\".*\"|.*='.*'")) #张牧之频道
@client.on(events.NewMessage(chats=[-1001285084157, my_chat_id], pattern=r".*=\".*\"|.*='.*'")) #自己的测试频道
@client.on(events.NewMessage(chats=[-1001235868507, my_chat_id], pattern=r".*=\".*\"|.*='.*'")) #diybot频道
...
async def myexport(event):
    try:
        ...
        if end:
            ...
            await cmd("jtask /jd/scripts/smiek_jd_zdjr.js now") # 组队瓜分京豆团ID更新后自动执行smiek_jd_zdjr.js脚本
    except Exception as e:
        await jdbot.send_message(chat_id, 'something wrong,I\'m sorry\n' + str(e))
        logger.error('something wrong,I\'m sorry\n' + str(e))
